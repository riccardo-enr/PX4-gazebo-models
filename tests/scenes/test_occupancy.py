...
def test_cylinder_centre_occupied():
    ...
    scene.add(SceneObject(name="c", shape=Cylinder(radius=0.4, length=1.0), position=(2.0, 2.0, 2.0)))
    grid = to_occupancy_grid(scene)
...
def test_cylinder_outside_radius_unoccupied():
    scene = Scene(name="cyl", extent=(4.0, 4.0, 4.0), resolution=0.5)
    scene.add(SceneObject(name="c", shape=Cylinder(radius=0.2, length=4.0), position=(2.0, 2.0, 2.0)))
    grid = to_occupancy_grid(scene)
...
def test_rotated_box_dtype_shape():
    ...
    scene.add(SceneObject(name="b", shape=Box(size=(1.0, 1.0, 1.0)), position=(2.0, 2.0, 2.0), rotation=q))
    grid = to_occupancy_grid(scene)
    assert grid.dtype == bool
