...
def main():
    ...
    parser.add_argument(
        '--avl_path',
        default=filedir,
        help='Provide an absolute AVL path. If this argument is passed, AVL will be moved there and the files will adjust their paths accordingly.',
    )
        ...
        case 'custom':
            ...
            for i, control_surface in enumerate(yaml_data['control_surfaces']):
                ...
                if ctrl_surf_type not in ctrl_surface_types:
                    raise ValueError(
                        f'The selected type is invalid. Available types are: {ctrl_surface_types}'
                    )
