...
def ctrl_surface_coef(file: TextIO, ctrl_surface_vec: list, index: str, direction: str):
    ...
    extracted_text = extracted_text.replace(
        '<direction></direction>', f'<directon>{direction}</direction>'
    )
    extracted_text = extracted_text.replace(
        '<CD_ctrl></CD_ctrl>', f'<CD_ctrl>{ctrl_surface_vec[0]}</CD_ctrl>'
    )
    extracted_text = extracted_text.replace(
        '<CY_ctrl></CY_ctrl>', f'<CY_ctrl>{ctrl_surface_vec[1]}</CY_ctrl>'
    )
    extracted_text = extracted_text.replace(
        '<CL_ctrl></CL_ctrl>', f'<CL_ctrl>{ctrl_surface_vec[2]}</CL_ctrl>'
    )
    extracted_text = extracted_text.replace(
        '<Cell_ctrl></Cell_ctrl>', f'<Cell_ctrl>{ctrl_surface_vec[3]}</Cell_ctrl>'
    )
    extracted_text = extracted_text.replace(
        '<Cem_ctrl></Cem_ctrl>', f'<Cem_ctrl>{ctrl_surface_vec[4]}</Cem_ctrl>'
    )
    extracted_text = extracted_text.replace(
        '<Cen_ctrl></Cen_ctrl>', f'<Cen_ctrl>{ctrl_surface_vec[5]}</Cen_ctrl>'
    )
...
def main(
    ...
    ref_pt_y: str,
    ref_pt_z: str,
    num_ctrl_surfaces: str,
    area: str,
    ctrl_surface_order: list,
    avl_path: str,
):
    ...
    if curr_path.returncode == 0:
        ...
        raise LookupError(
            'Invalid path to directory. Check both the avl_automation directory and the Avl directory are positioned correctly.'
        )
    ...
    match plane_type:
        case 'custom':
            for i, ctrl_surface in enumerate(ctrl_surface_order):
                ...
                ctrl_surface_coef(
                    file_name, ctrl_surface_mat[seen_index], i, ctrl_direction[ctrl_surface]
                )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'file_name', help='The file to which the desired coefficients should be written.'
    )
    ...
    parser.add_argument(
        'ref_pt_x',
        help='The x coordinate of the reference point, at which forces and moments are applied.',
    )
    parser.add_argument(
        'ref_pt_y',
        help='The y coordinate of the reference point, at which forces and moments are applied.',
    )
    parser.add_argument(
        'ref_pt_z',
        help='The z coordinate of the reference point, at which forces and moments are applied.',
    )
    parser.add_argument(
        'num_ctrl_surfaces', help='The number of control surfaces that the model uses.'
    )
    parser.add_argument('area', help='The wing surface area.')
    parser.add_argument(
        'ctrl_surface_order',
        help=' A list containing the types of control surfaces, in theorder in which \
    	they have been defined in the .avl file.',
    )
    parser.add_argument(
        'avl_path',
        help='A string containing the directory where the AVL directory should be moved to.',
    )
    ...
    main(
        ...
        args.ref_pt_y,
        args.ref_pt_z,
        args.num_ctrl_surfaces,
        args.area,
        args.ctrl_surface_order,
        args.avl_path,
    )
